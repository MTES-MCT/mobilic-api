...
class TestReportInfractions(RegulationsTest):
    ...
    def test_no_lic_infraction_not_added_when_ongoing_activity_at_control_time(
        self,
    ):
        """
        Test that NO_LIC infraction is NOT added when there is an ongoing activity at control time.
        """
        trm_business = Business.query.filter(
            Business.transport_type == TransportType.TRM
        ).first()
        ...
        control = ControllerControlFactory.create(
            user_id=self.employee.id,
            controller_id=self.controller_user.id,
            qr_code_generation_time=control_time,
            control_bulletin={"business_id": trm_business.id},
        )

        # Create an activity that starts before and ends after control time (ongoing)
        start = control_time - datetime.timedelta(hours=2)
        end = control_time + datetime.timedelta(hours=2)
    ...
    def test_no_lic_infraction_added_when_activity_ended_before_control_time(
        self,
    ):
        """
        Test that NO_LIC infraction IS added when activity ended before control time.
        """
        trm_business = Business.query.filter(
            Business.transport_type == TransportType.TRM
        ).first()
        ...
        # Create an activity that ended before control time
        start = control_time - datetime.timedelta(hours=4)
        end = control_time - datetime.timedelta(hours=1)
