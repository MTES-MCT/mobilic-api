...
def generate_control_bulletin_pdf(control):
    ...
    if (
        controller
        and controller._is_mi()
        and not (controller.greco_id and controller.greco_id.strip())
    ):
...
def _generate_part_one(control):
    ...
    business_types = ", ".join(
        list(set([b.transport_type.name for b in businesses]))
    )
    ...
    if not control.controller_user:
        ...
        if controller._is_ctt() or controller._is_mi():
            ...
            controller_signature = (
                f"{controller.last_name} {controller.first_name}"
            )

    birth_date_str = control.control_bulletin.get("user_birth_date")
    formatted_birth_date = (
        datetime.strptime(birth_date_str, "%Y-%m-%d").strftime("%d/%m/%Y")
        if birth_date_str
        else "-"
    )
   ...
   # Détermine la valeur du poids de véhicule à afficher (Poids réel "real_vehicle_weight", PTAC, PV ou '-' si pas de valeur renseignée)
    if control.control_bulletin and control.control_bulletin.get("real_vehicle_weight") is not None:
        ...
        if value is not None:
            formatedValue = str(value).replace(".", ",")
        printedVehicleWeightValue = f"{formatedValue} tonnes"
    ...
    return generate_pdf_from_template(
        ...
        controlled_employee_nationality=control.control_bulletin.get(
            "user_nationality"
        ),
